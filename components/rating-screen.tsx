...
export function RatingScreen({
        ...
        if (userType === "passenger") {
          ...
          const tripsWithPassengers = await Promise.all(
            allTrips.map(async (trip: Trip) => {
              try {
                const passengersRes = await fetch(`http://rumbo-back-production.up.railway.app/api/trips/${trip.id}/passengers`)
                if (passengersRes.ok) {
            ...
            completedTrips.map(async (trip: Trip) => {
              try {
                const passengersRes = await fetch(
                  `http://rumbo-back-production.up.railway.app/api/trips/${trip.id}/passengers`
                )
                if (passengersRes.ok) {
                  try {
                    const passengersData = await passengersRes.json()
  ...
  const handleRate = async (tripId: string, targetUserId: string) => {
    ...
    try {
      ...
      const res = await fetch(`http://rumbo-back-production.up.railway.app/api/users/${targetUserId}/ratings`, {
        method: "POST",
        headers: {
            ...
            onClick={() => {
              ...
              const fetchData = async () => {
                try {
                  ...
                  if (userType === "passenger") {
                    const tripsRes = await fetch(`http://rumbo-back-production.up.railway.app/api/trips/users/${userId}`)
